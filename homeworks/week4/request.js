...
// 因此，這週的超級挑戰題就是讓你試試看能否不用 request，只用 Node.js 內建的套件來發出 request，並且改寫 hw2，改寫成只用原生的 library。
...
request.delete = function (url, callback) {
  sendRequest(url, { method: "DELETE" }, callback);
};

request.post = function ({ url, form }, callback) {
  sendRequest(url, { method: "POST", data: form }, callback);
};

request.patch = function ({ url, form }, callback) {
  sendRequest(url, { method: "PATCH", data: form }, callback);
};
